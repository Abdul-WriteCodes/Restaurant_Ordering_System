...
PLANS = [
    {
        ...
        "features": ["Unlimited EFA & CFA runs","10 dataset exports",
                     ...
                     "HTML analysis report","Access key via email"],
    ...
    },
    {
        ...
        "features": ["Unlimited EFA & CFA runs","30 dataset exports",
                     ...
                     "HTML analysis report","ZIP bundle export","Access key via email"],
    ...
    },
    {
        ...
        "features": ["Unlimited EFA & CFA runs","100 dataset exports",
                     ...
                     "HTML analysis report","ZIP bundle export",
...
def render_landing():
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;
      ...
      <div style="background:#111520;border:1px solid #1e2540;border-radius:14px;padding:28px 22px;text-align:center;">
        ...
        <div style="font-family:Inter,sans-serif;font-size:13px;color:#64748b;line-height:1.7;">
          Download cleaned data, synthetic datasets and a full HTML analysis report.
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="max-width:860px;margin:0 auto 16px;padding:0 4px;">
      <div style="background:linear-gradient(135deg,rgba(108,141,250,0.07),rgba(167,139,250,0.06));
        ...
        <div style="display:flex;flex-wrap:wrap;justify-content:center;gap:10px;">
          ...
          <span style="background:rgba(248,113,113,.07);border:1px solid rgba(248,113,113,.2);color:#f87171;
                       font-family:Inter,sans-serif;font-size:11px;font-weight:500;padding:6px 14px;border-radius:8px;opacity:.85;">✗ HTML report (requires credits)</span>
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
# HTML REPORT GENERATOR  (core logic unchanged)
# ══════════════════════════════════════════════════════════════════
...
<title>EFActor — Analysis Report</title>
<style>
:root{{--bg:#0a0c14;--surface:#111520;--border:#1e2540;--accent:#6c8dfa;
  --accent2:#a78bfa;--green:#34d399;--red:#f87171;--text:#e2e8f0;--muted:#64748b;}}
*{{box-sizing:border-box;margin:0;padding:0;}}
body{{background:var(--bg);color:var(--text);font-family:Inter,system-ui,sans-serif;line-height:1.7;padding:40px 20px;}}
.container{{max-width:960px;margin:0 auto;}}
h1{{font-size:2rem;color:var(--accent);margin-bottom:4px;font-weight:800;}}
h2{{font-size:1rem;color:var(--accent2);margin:32px 0 12px;border-bottom:1px solid var(--border);
  padding-bottom:6px;text-transform:uppercase;letter-spacing:1.5px;font-weight:700;}}
.meta{{color:var(--muted);font-size:.88rem;margin-bottom:28px;}}
.card{{background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:18px 22px;margin-bottom:14px;}}
.badge{{display:inline-block;padding:3px 12px;border-radius:20px;font-size:.78rem;font-weight:600;}}
.bp{{background:rgba(52,211,153,.12);color:var(--green);border:1px solid rgba(52,211,153,.35);}}
.bf{{background:rgba(248,113,113,.12);color:var(--red);border:1px solid rgba(248,113,113,.35);}}
.stat-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:10px;margin-top:10px;}}
.stat-box{{background:var(--border);border-radius:8px;padding:14px 16px;}}
...
th{{background:var(--border);color:var(--accent);padding:8px 10px;text-align:left;
  font-size:.75rem;text-transform:uppercase;letter-spacing:1px;font-weight:600;}}
td{{padding:7px 10px;border-bottom:1px solid var(--border);}}
tr:last-child td{{border-bottom:none;}}
pre{{background:#0d1020;border:1px solid var(--border);padding:14px;border-radius:8px;
  font-family:'JetBrains Mono','Courier New',monospace;font-size:.82rem;color:#a5f3fc;overflow-x:auto;}}
footer{{color:var(--muted);font-size:.78rem;margin-top:48px;text-align:center;}}
hr{{border:none;border-top:1px solid var(--border);margin:28px 0;}}
</style></head><body><div class="container">
<h1>◈ EFActor — Analysis Report</h1>
...
<div class="card" style="margin-top:12px"><strong>Dropped Variables:</strong>
  <span style="color:var(--muted)">{', '.join(dropped_vars) if dropped_vars else 'None'}</span></div>
...
"""
    ...
    load_rows = "".join(
        ...
        f"{communalities[v]:.3f}</td></tr>" for v in loadings.index)
    var_rows = "".join(
...
<div class="card" style="overflow-x:auto">
  <table><thead><tr><th>Variable</th>{load_headers}<th>Communality</th></tr></thead><tbody>{load_rows}</tbody></table></div>
<div class="card" style="overflow-x:auto">
  <table><thead><tr><th>Factor</th><th>SS Loadings</th><th>Proportion Var</th><th>Cumulative Var</th></tr></thead>
  <tbody>{var_rows}</tbody></table></div>
<div class="card" style="overflow-x:auto">
...
<div class="card">
  <div style="margin-bottom:12px">Model Fit: <span class="badge {ov_b2}">{'ADEQUATE' if fa['overall_pass'] else 'INADEQUATE'}</span>
    &nbsp;<span style="color:var(--muted);font-size:.85rem">{fa['n_pass']}/{fa['n_total']} indices passed</span></div>
  <table><thead><tr><th>Index</th><th>Value</th><th>Threshold</th><th>Result</th></tr></thead>
  <tbody>{fit_rows}</tbody></table></div>
<h2 style="font-size:.9rem">Model Specification</h2><pre>{model_str}</pre>
"""
    if synthetic_validation is not None:
        syn_rows = "".join(
            f"<tr><td>{r['Variable']}</td><td>{r['OrigMean']}</td><td>{r['SynMean']}</td>"
            f"<td>{r['OrigStd']}</td><td>{r['SynStd']}</td><td>{r['MeanDelta']}</td><td>{r['StdDelta']}</td></tr>"
...
<div class="card" style="overflow-x:auto">
  ...
  <tbody>{syn_rows}</tbody></table></div>
"""
    html += """<hr><footer>EFActor — Results should be interpreted in context of your research design and theoretical framework.</footer>
</div></body></html>"""
    return html
    ...
    if S.get("_last_file_key") != _file_key:
        try:
            ...
            for k in ["suitability","efa_result","cfa_result","fit_assessment","efa_done",
                      "synthetic_factor","synthetic_corr","syn_validation","report_html",
...
if S.suitability is None:
    ...
    n_factors = st.number_input("Number of factors to extract", 1,
                                 max(2, len(S.df_working.columns)-1), default_n, 1)
...
if S.efa_result is None:
    ...
    st.dataframe(S.diagnostics, use_container_width=True, hide_index=True)
...
if S.efa_result is None:
    ...
    with col_e3:
        ...
        if has_cfa:
            st.markdown(f'<div class="info-box" style="font-size:.82rem;">Full EFA + CFA report<br><b>Cost: {REPORT_COST} credit</b></div>', unsafe_allow_html=True)
            if current_credits < REPORT_COST:
                ...
                if st.button(f"🔨 Build HTML Report ({REPORT_COST} cr)", use_container_width=True, key="build_rpt"):
                ...
                if S.report_html:
                    st.download_button("⬇ efa_cfa_report.html",
                                       data=S.report_html.encode("utf-8"),
                                       file_name="efactor_report.html", mime="text/html",
    ...
    zip_cost = export_credit_cost(zip_rows) + (REPORT_COST if S.report_html else 0)
    st.markdown(f'<div class="info-box" style="font-size:.82rem;">Cleaned data + synthetic data + HTML report + loadings CSV + model spec<br><b>Cost: {zip_cost} credits</b></div>', unsafe_allow_html=True)
